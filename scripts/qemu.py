...
print(f"[{qemu}]")
os.system(f"{qemu} {qemu_flags} -m {qemu_memory} -bios {bios} -serial stdio -s -net none -device virtio-blk-pci,drive=hdd -drive id=hdd,if=none,format=raw,file=fat:rw:image")