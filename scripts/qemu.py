...
print(f"[{qemu}]")
os.system(f"{qemu} {qemu_flags} -no-reboot -m {qemu_memory} -bios {bios} --serial stdio -s -net none -drive format=raw,file=fat:rw:image,id=nvm,if=none -device nvme,serial=deadbeef,drive=nvm")
